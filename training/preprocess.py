...
FILTER_HFREQ = 30.0     # Low-pass filter frequency (Hz)
...
# --- Data Saving ---
SAVE_DIR = './processed_data_ica' # Directory to save processed data
EPOCHS_SAVE_PATH = os.path.join(SAVE_DIR, f'{os.path.basename(set_file_path).split(".")[0]}_epochs_ica_std.npy')
LABELS_SAVE_PATH = os.path.join(SAVE_DIR, f'{os.path.basename(set_file_path).split(".")[0]}_labels.npy')
...
try:
    raw = mne.io.read_raw_eeglab(set_file_path, preload=True, verbose=False) # Load with preload=True
    ...
    for condition in existing_conditions:
         try:
             # Plot average of the *original* scaled epochs
             evoked = epochs[condition].average() # No need to pick_types here if epochs were created with picks='eeg'
